...
if (empty($notice_content)) {
    ...
    ?><br/>
			<?php 
    echo \wp_kses_post(\__('We need your help to improve <strong>WP Desk plugins</strong>, so they are more useful for you and the rest of <strong>30,000+ users</strong>. By collecting data on how you use our plugins, you will help us a lot. We will not collect any sensitive data, so you can feel safe.', 'shopmagic-for-twilio'));
    ?>
			<a href="<?php 
...
\esc_html_e('Enable usage tracking', 'shopmagic-for-twilio');
?></button>
	</p>
</div>
...
?>',
			{
				...
				data: {
					security: '<?php 
echo \esc_attr(\wp_create_nonce(\ShopMagicTwilioVendor\WPDesk_Tracker::WPDESK_TRACKER_NOTICE));
?>',
